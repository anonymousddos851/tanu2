...
def configure_ngrok():
    ngrok_token = "2ltPP4qpfxpG1PXnB2xvdfkLcRz_5FTsndUvFcGxTVmBQAhfQ"
    try:
...
def update_soul_txt(public_url):
    with open("kumarx1.txt", "w") as file:
        file.write(public_url)
    print(f"New ngrok link saved in kumarx1.txt")
...
def update_vps_soul_txt(public_url):
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("kumarx1.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated kumarx1.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update kumarx1.txt on VPS: {str(e)}")
